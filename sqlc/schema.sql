...
DROP TABLE IF EXISTS track_musicians CASCADE;
...
DROP TABLE IF EXISTS spotify_images CASCADE;
...
CREATE TABLE users (
  ...
  avatar TEXT NOT NULL DEFAULT '/static/images/no_avatar.png'
);
...
CREATE TABLE spotify_images (
  ...
  CHECK (
    (musician_id IS NOT NULL AND album_id IS NULL) OR
    (musician_id IS NULL AND album_id IS NOT NULL)
  )
);
...
CREATE TABLE tracks (
  ...
  bit_depth INTEGER,
  album_id INTEGER REFERENCES albums(id) ON DELETE CASCADE,
  CONSTRAINT chk_tracks_nonneg CHECK (duration > 0 AND channels >= 1)
);
...
CREATE INDEX idx_track_musicians_track_id ON track_musicians (track_id);
CREATE INDEX idx_track_musicians_musician_id ON track_musicians (musician_id);
...
CREATE INDEX idx_spotify_images_path ON spotify_images (path);

-- Composite indexes for common patterns
CREATE INDEX IF NOT EXISTS idx_cast_movie_order ON cast_list (movie_id, sort_order);
CREATE INDEX IF NOT EXISTS idx_tracks_album_idx ON tracks (album_id, index);

-- Trigram indexes for fast ILIKE/fuzzy search
...
-- Tracks: unique track number per album
ALTER TABLE tracks ADD CONSTRAINT uq_album_tracknum UNIQUE (album_id, index);
