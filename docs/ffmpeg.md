...
The TV scanner (`server/cmd/internal/scanner/show`) runs at startup and through admin `POST /api/settings/scan/shows`, using the `shows_dir` captured when starting. `SHOWS_DIR` seeds this setting on first launch. Saving library settings does not launch scans. The scanner shares the scan guard pattern, shutdown context and wait group, batching, 60-second quiet period, full-file fingerprints, write mutex, and safe reconciliation used by the existing scanners.

Discovery requires `Show Name (optional year)/Season N/file` or `Show Name/Specials/file`. Case-insensitive `S01E02`, inclusive `S01E02-E04`, enumerated `S01E02E03`, and `1x02` tokens are accepted. Specials use season zero. Conflicting or malformed numbering, reversed ranges, and filename/directory season mismatches are logged and rejected. Absolute-number and date-based numbering are not inferred. Hidden entries (including backup directories) and nested extras are excluded. NFO files and external subtitle sidecars are ignored. Video extensions and file/directory symlink behavior match movies.

Cleaned absolute show-directory paths identify shows; separate folders never merge by TMDB ID. Seasons and logical episodes exist only for successfully probed files. A combined file links multiple episodes in filename order; duplicate copies link the same logical episode. Each new or changed physical file is probed once with `GetMetadata`, using movie technical conversions and video acceptance rules, including moving MJPEG and attached-artwork rejection. Video, audio, subtitles, chapters, and fingerprints belong to the file. Absolute ffprobe indices are retained. File duration is never divided into guessed episode runtimes or start offsets. Episode runtime is TMDB metadata only. File updates retain the file ID and commit local fallbacks, technical rows, links, and enrichment markers atomically. Fingerprint-only updates retain technical rows.

After technical processing and safe cleanup, eligible pending enrichment runs sequentially in show/season groups. TV search uses the folder title and optional premiere year with unfiltered fallback, movie title ranking and ambiguous-year interpretations, deterministic ties, and no confidence cutoff. Stored show IDs are fetched directly and never automatically rematched. Show details include aggregate credits, content ratings, external IDs, and videos; season details supply aggregate credits, videos, and episode descriptions. Episode credits are requested only for eligible local episodes. Missing episodes remain pending. Successful show/season credits preserve every character and job; episode regular cast, guest cast, and crew are separate. US certification is preferred with a nonempty-country fallback; metadata defaults to English. Remote artwork paths and trailer metadata are stored without downloads or thumbnails.

Each entity's required responses must succeed before its metadata and relationships replace prior values transactionally. Requests run outside the database mutex; persistence rechecks the show path and all owning IDs, numbers, and observed TMDB identities. Failed show lookups defer descendants while other shows continue. Each entity is attempted at most once per scan; a season response is reused for its local episodes. Success clears retries; failures and unavailable TMDB retain them for later scans. Metadata-only retries do not probe or rebuild technical rows. Successful unchanged entities are skipped, and file changes queue their show, season, and episodes again. TV response caches use separate keys and are cleared with other TMDB caches.

Cleanup follows the root-identity and confirmed-nonexistence checks below. File deletion cascades through links and technical rows; the same transaction prunes episodes with no files, seasons with no files, and shows with no seasons. Removing one copy preserves episodes with other copies. Shared people, genres, companies, networks, and extra videos remain. Observed failures and deferred files are protected. Renames import a new path and clean up the missing old path. Completion logs distinguish imported, updated, skipped, deferred, rejected, and deleted files from local episode and enrichment counts. TMDB season/episode totals are stored separately from available counts derived from file relationships.

TV browsing, playback, watch progress, web scan controls, manual Identify, alternate/DVD numbering, NFO ingestion, filesystem watching, and background metadata refresh are deferred.
...
Startup and manual scans capture the configured directory and reconcile missing catalog files after a successful walk and final batch, before music Spotify retries. Candidates include rows without fingerprints and are restricted by cleaned absolute path containment within that directory. Files observed by the walker are protected before processing, including unchanged, deferred, and failed files. A file observed and then deleted is reconciled on the next scan.
...
All three batch processors call the same single-path detector in `scanner`. The cleaned catalog path is the lookup key. A successful fingerprint contains filesystem size, nanosecond modification and status-change times, device and inode identity, and a full-file SHA-256 digest, including embedded tags and artwork. Complete matching filesystem metadata skips hashing and probing. A new file or missing baseline is hashed and processed normally. Changed metadata triggers sequential full hashing with a reusable 256 KiB buffer and cancellation checks. If size and digest still match, the technical update only changes the fingerprint: catalog timestamps, relationships, and playback caches are preserved unless the movie also has pending descriptive enrichment, which follows the metadata-only retry path described above. Changed bytes are probed and resolved normally, retaining the catalog ID at that path.

Files must be quiet for 60 seconds after the later of modification and status-change time. Future-dated files remain deferred. The detector reports a typed outcome, observed fingerprint, deferral reason, and, for quiet-period deferrals, the earliest eligible time. The descriptor used for hashing remains open through processing. Regular-file type, descriptor/path identity, size, timestamps, and symlink resolution are checked around hashing, after metadata resolution, and immediately before commit. Detected writes, disappearance, replacement, or symlink retargeting defer the file while preserving the previous catalog and baseline. Read, hash, probe, and database failures are reported as failures and also preserve the baseline. Deferred totals are logged separately from failures; cancellation never reports successful scan completion.

Internal `track_file_fingerprints` and `movie_file_fingerprints` tables cascade with their catalog rows. Timestamps are integers, unsigned filesystem identifiers are decimal text, and SHA-256 is a 32-byte blob; size remains on the catalog row. Changed-file fingerprints commit in the existing per-item metadata transaction. In-memory baselines and runtime cache invalidation are published only after commit. Movie reprocessing also removes persisted remux verdicts and keyframe indexes in that transaction. Hashing, probing, and external metadata lookups run outside the shared scanner database mutex.

Detection owns no scheduling or scan-lifetime state. Current scans serialize their own processing; future watcher orchestration must coordinate with scans before processing concurrently. A future fsnotify handler can inspect affected paths through the same detector, debounce repeated write notifications, and schedule quiet-period retries. Events trigger inspection and never replace a successful persisted baseline. fsnotify installation, event queues, automatic retries, event-driven deletion handling, move recognition, and duplicate merging are not implemented. Deferred files currently retry on the next startup or manual scan. Full scans remain necessary for startup and recovery, including changes while Igloo is stopped and notification queue overflow.

This detection targets local disks on Linux x64 and macOS ARM64. The initial baseline reads every eligible file completely. Metadata-based fast checks cannot detect metadata-invisible corruption, and stability checks cannot prove that a paused copy is complete.
