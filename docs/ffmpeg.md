...
Personal HLS sessions are keyed by movie ID, requested profile, audio track, playback session ID, and start time. If the same request arrives again, Igloo refreshes the cached session TTL and reuses the process. When a new personal session is created for the same movie and user, all of that user's older sessions for the movie are cleaned up — including sessions from other playback session IDs — so seeks, reloads, and reopened browsers do not leave extra FFmpeg processes running or holding transcode permits. Before creating a session, Igloo also reclaims the owner's stale sessions for the movie and evicts expired cache entries so an abandoned session's transcode permit is released ahead of the new session's acquire. The same movie is therefore never served to one user by two live personal sessions; different movies played concurrently by the same user keep their sessions. Concurrent creation is deduplicated with singleflight so multiple near-simultaneous manifest requests do not start duplicate transcodes for the same session. Clients can also tear a playback session's HLS sessions down explicitly with `POST /api/movies/{id}/hls/session/stop`; the stop endpoint stays scoped to its own playback session ID so a late stop from a closing tab cannot remove a session the user just created after reopening.

Personal sessions use a 5-minute idle TTL that every manifest and segment request refreshes. Because hls.js stops fetching once its buffer is full and a paused tab fetches nothing, the web player refetches the manifest every 2 minutes while mounted as a keepalive; a client that skips the keepalive (or wakes from OS sleep after eviction) recovers transparently because a manifest request recreates the session at the same start offset. Watch-room sessions keep a 30-minute TTL — rooms have no per-client keepalive and always warm from the beginning, so evicting an idle room would restart playback for every participant. The cache sweep runs every minute, so an abandoned personal session is fully reclaimed (FFmpeg killed, temp dir removed, transcode permit released) within about six minutes even without an explicit stop.
...
The HLS manifest accepts a `start` query parameter. When `start` is greater than zero, FFmpeg starts from that source offset with `-ss`. A start offset at or past the movie's duration — stale saved progress after a re-scan, or rounding at the very end — is clamped to five seconds before the end instead of failing, so resume always produces a playable session.
