...
Igloo uses Jellyfin FFmpeg builds for release payloads, never generic upstream builds. The Linux x64 and macOS ARM64 payloads are from the stable Jellyfin FFmpeg `v8.1.2-5` release and report `8.1.2-Jellyfin`. Their portable archive SHA-256 digests are `1fd859927053c44a4f2dbf67ae8b9ba8d29fb3b8930df0dd57d91aa60589363d` (Linux x64) and `b2ac80bb184e9a2f3f7c236876b2f56a5639596a95b42f41ced34fab66ad720d` (macOS ARM64). Follow the current stable `jellyfin-ffmpeg` line; do not move to a prerelease line or an upstream build without a specific reason.
...
The web client applies that 120-second rule to every seek the video element makes, not only to the ones its own controls issue, and measures it from where playback last came to rest rather than from `currentTime`. A picture-in-picture or native fullscreen scrubber writes `currentTime` directly, and during a slider drag or a run of key presses `currentTime` is already the previous pending target, so chained seeks used to walk far past the encoder one step at a time. Either way the player then asked a transcode session for a segment minutes of encoding away: the server long-polled it for the full segment wait, answered `503`, and hls.js and the player's retries repeated that for tens of minutes. The rule reads the element's own seek events, so it covers the browser's native HLS engine (iPhone Safari) as well as hls.js. The player reports a run of seeks once, from the settled point to where the run ended, after the element has been quiet for 250 ms (`HLS_SEEK_SETTLE_MS`), so a native scrub or a held-down key costs one rebase at its final position rather than one per 120 seconds of travel, each of which would have started a transcode. The page's own seek bar, keys, and buttons wait out the same window before a rebase they call for: the seek bar seeks on every pointer move, and a rebase moves the session start at once, so a drag used to rebase on any step that crossed the 120-second window and, dragging back past the new session's start, on every ten seconds or so of travel. A pending rebase leaves the element where it is and shows its target on the controls, a later step re-arms it, and a step back into range drops it, so a far click takes effect 250 ms later and a drag costs one rebase at the position it ended on. It lives entirely in the web client's playback page: the server contract is unchanged, and the watch room, which cannot rebase, does not use it.
...
NVIDIA encode is checked with a short runtime encode probe, not just by looking for `h264_nvenc` in `ffmpeg -encoders`. The NVIDIA runtime probes (encode, CUDA scale, CUDA tone map) encode a 256×144 test frame, because NVENC rejects an H.264 frame below a minimum that depends on the GPU and driver (145×49 on the QA server's GTX 1660 SUPER) and 256×144 clears the minimums NVIDIA reports, and the tone-map probe tags its frame as PQ with BT.2020 primaries, because `tonemap_cuda` refuses a frame with no HDR transfer characteristic. A probe that gets either wrong fails on hardware that works: a failed encode probe sends every transcode to `libx264` (the session-start log shows `transcode_pool=cpu` and a `hw_fallback_reason`), and a failed scale or tone-map probe quietly moves that step onto the CPU filters. When the probed build supports the `cuda` hwaccel, NVIDIA transcodes add `-hwaccel cuda` without `-hwaccel_output_format`: FFmpeg decodes on the GPU when the source codec is supported and transparently falls back to software decode otherwise, and decoded frames land in system memory either way, so the same filter chains work in both cases. For SDR transcodes, NVIDIA normally uses software scaling into `yuv420p` frames before `h264_nvenc` encode:
