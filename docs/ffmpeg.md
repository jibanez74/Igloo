...
Detection owns no scheduling or scan-lifetime state. Current scans serialize their own processing; future watcher orchestration must coordinate with scans before processing concurrently. A future fsnotify handler can inspect affected paths through the same detector, debounce repeated write notifications, and schedule quiet-period retries. Events trigger inspection and never replace a successful persisted baseline. fsnotify installation, event queues, automatic retries, deletion handling, move recognition, and duplicate merging are not implemented. Deferred files currently retry on the next startup or manual scan. Full scans remain necessary for startup and recovery, including changes while Igloo is stopped and notification queue overflow.
