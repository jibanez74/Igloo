...
Clients treat a segment `404` as a lost session and rebase, with one exception: a `404` on the final segment of a playlist that has ended is the end of the media. The synthesized transcode playlist is sized from the last video frame (see "HLS Output Format"), but a source whose audio outlasts its video by more than a frame can still leave it one segment longer than FFmpeg writes; everything before that segment is buffered by then, so the web client stops loading and signals end of stream (`BUFFER_EOS`) instead of rebasing the session three times and reporting the film as unrecoverable. A native client should apply the same rule.
