...
export default function UsersTable(props: UsersTableProps) {
  ...
  const Route = getRouteApi("/_auth/_admin/users/");
  ...
  const mutation = createMutation(() => ({
    mutationFn: async () => {
      try {
        return fetch(`/api/v1/users/delete/${id()}`, {
          method: "delete",
  ...
  const handleDeleteClick = (user: SimpleUser) => {
    const msg = `Are you sure you want to delete user ${user.name}? This action cannot be undone.`;
      ...
      <div role="region" aria-label="Users" class="overflow-x-auto">
        ...
        >
          <caption id="users-table-description" class="sr-only">
            List of system users with their details including name, username,
            email, status, role, and available actions
          </caption>
