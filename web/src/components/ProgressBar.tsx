...
type ProgressBarVariant = "expanded" | "minimized" | "mobile";
...
> = {
  expanded: {
    ...
    timeText: "text-sm text-slate-500 tabular-nums",
    showTimes: true,
    timesLayout: "below",
  },
  minimized: {
  ...
  mobile: {
    ...
    timeText: "text-xs text-slate-500 tabular-nums",
    showTimes: true,
    timesLayout: "below",
  },
};
...
export default function ProgressBar({
  ...
  if (styles.timesLayout === "inline") {
    return (
      <div className={styles.container} role="group" aria-label="Playback progress">
  ...
  // Render below layout (expanded and mobile)
  return (
    <div className={styles.container} role="group" aria-label="Playback progress">
      <div
        onClick={handleClick}
      ...
      {styles.showTimes && (
        <div className={variant === "mobile" ? "mt-1 flex justify-between" : "mt-2 flex justify-between"}>
          <span className={styles.timeText} aria-hidden="true">
            {formatTimeSeconds(currentTime)}
          </span>
          <span className={styles.timeText} aria-hidden="true">
