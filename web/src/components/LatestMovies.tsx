    ...
    <section class="py-8">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-10 mb-6">
          <h2 class="text-2xl font-bold text-white">Latest Movies</h2>
            ...
            fallback={
              <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 sm:gap-6">
                <For each={Array(12).fill(null)}>
                  {() => (
                    <div class="aspect-[2/3] rounded-xl bg-slate-800/50 animate-pulse" />
                  )}
                </For>
              </div>
            }
            ...
            <Switch>
              <Match when={query.data?.movies.length === 0}>
                <div class="h-full flex items-center justify-center">
                  <p class="text-blue-200">No movies available</p>
                </div>
              </Match>
