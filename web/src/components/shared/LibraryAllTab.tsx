...
// The "everything" tab of a library page: one paginated, sortable poster grid.
// Navigation stays with the page (it owns the typed route search), so the tab
// only reports what the user asked for.
...
>({
  ...
  if (isLoading) {
    return <LibraryAllTabSkeleton perPage={perPage} />;
  }
...
// the same 2:3 boxes, one per card on a full page.
export function LibraryAllTabSkeleton({ perPage }: { perPage: number }) {
  return (
    <div>
