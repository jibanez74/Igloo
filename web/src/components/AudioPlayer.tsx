...
export default function AudioPlayer({
  ...
  // Render expanded (fullscreen) mode
  if (isExpanded) {
    return (
      <div
          ...
          >
            ...
            >
              {isLoading ? (
                ...
                <i className='fa-solid fa-pause text-3xl' aria-hidden='true' />
              ) : (
                <i
  ...
  // Render minimized (bottom bar) mode
  return (
    <div
      role='region'
      aria-label='Audio player'
      className='fixed bottom-0 left-0 right-0 z-50 bg-slate-900/95 backdrop-blur-lg border-t border-slate-700/50 shadow-2xl shadow-black/50'
    >
      {/* Hidden audio element */}
      <audio ref={audioRef} preload='metadata'>
        {streamUrl && <source src={streamUrl} type={track.mime_type} />}
