...
function PlaylistForm({ mode, playlist, onOpenChange }: PlaylistFormProps) {
  ...
  const [description, setDescription] = useState(
    playlist?.description?.Valid ? playlist.description.String : ""
  );
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data.error) {
        toast.error(config.errorMessage, { description: data.message });
        return;
      }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (name.trim().length > 255) {
      toast.error("Playlist name is too long (max 255 characters)");
      return;
    }
