...
func (c *musicBrainzClient) searchAlbum(title, artistHint string) (*AlbumResult, error) {
  query := fmt.Sprintf(`releasegroup:"%s"`, escapeQueryQuoted(title))
  if artistHint != "" {
    query += fmt.Sprintf(` AND artist:"%s"`, escapeQueryQuoted(artistHint))
  }
  ...
  return &AlbumResult{
    ...
    Year:          year,
    CoverURL:      fmt.Sprintf("%s/%s/front-500", helpers.COVER_ART_ARCHIVE_BASE_URL, rg.ID),
